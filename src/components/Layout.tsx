...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Pages that have a large hero image at the top allowing for transparent header initially
  const hasHeroImage = ['/', '/boende', '/kontakt'].includes(location.pathname);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`font-medium transition-colors hover:text-brand-lightGreen ${
                    isActive
        ...
        {/* Mobile Nav Overlay */}
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white shadow-lg border-t border-gray-100 flex flex-col p-4 animate-in slide-in-from-top-2">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`py-3 text-lg font-medium border-b border-gray-50 last:border-0 ${
                   location.pathname === item.path ? 'text-brand-green' : 'text-gray-600'
                }`}
              >
                {item.label}
              </Link>
